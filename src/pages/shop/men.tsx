...
const MEN_CATEGORIES = ["Clothes", "Shoes"];
...
export default function ShopMenPage() {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        const menProducts = data.filter((p) =>
          MEN_CATEGORIES.includes(p.category.name)
        );
        setProducts(menProducts);
      } catch (err) {
        setError("Gagal memuat produk pria.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <DefaultLayout title="Toko | Koleksi Pria">
      <section className="px-6 md:px-10 mt-10 mb-20">
        <div className="text-center mb-14">
          ...
          <p className="mt-4 text-lg text-neutral-600 max-w-2xl mx-auto">
            Temukan gaya Anda dengan koleksi pakaian dan aksesoris pria terbaru.
          </p>
        </div>
